...
1. `rvest` Wickham (2016b) for web scrapping

1. `tidyr` Wickham and Henry (2018) for data cleaning

1. `stringr` Wickham (2018) for string manipulation.

1. `shiny` Chang, Cheng, Allaire, Xie, & McPherson (2018) for creating shiny app

1. `RSelenium` Harrison (2018) for doing search live

1. `XML` Lang and the CRAN Team (2018) for doing assistant work of search live

1. `sqldf` Grothendieck (2017) for selecting desired data

1. `forecast` Hyndman et al (2018) for predicting prices using arima model
...
To properly drive this application, the user should first make sure connection to the internet, then run all R Scripts before opening the `app.R`. This is to supply all function calls that will happen in the server of the shiny app while operating. A further improvement arises here as we want to simply this process and write all functions into a package. In doing this, the user can directly run the `app.R` and have all the functionalities work appropriately. 
...
As a result, we successfully designed the shiny app that have three different functions: predict,comparison and search live. In the prediction function panel, we are able to predict the future prices of a certain shoe among the default options and also predict  the time when the max or minimum price will be reached. To make it more clear for our users, we provides with the picture for the shoe, the prediction graph with max and minimum prices marked and the text description about the prediction. In the comparison panel, we are able to compare the sales of two different kinds of shoes using graphs to better understand the popularity of certain sneakers. Finally in the search live function we are able to get the latest information about prices and available sizes for the shoes we type in the searching bar.
